...
class Tetromino {
  ...
  static Tetromino sQuare()    { 
    ...
    static Tetromino n()          {//[B2] here is where I actually create my new piece
    return new Tetromino(new Point(1, -2),
                         makeBiggerBlocks(new int[] {0, -1, 2, -1, 0, -2, 1, -2, 2, -2},
                         Color.CYAN    ));
  }
  ...
  //[B1] a different makeBlocks for tetrominos with 5 blocks
  static SetOfBlocks makeBiggerBlocks(int[] c, Color color){
    SetOfBlocks b = new SetOfBlocks();
    b.add(new Block( c[0], c[1], color));
    b.add(new Block( c[2], c[3], color));
    b.add(new Block( c[4], c[5], color));
    b.add(new Block( c[6], c[7], color));
    b.add(new Block( c[8], c[9], color));
    return b;  
  }
  Point center;
  SetOfBlocks blocks;
  ...
  static Tetromino pickRandom() {
    int value = (int)(Math.random() * 8); //[B3] changed the Math.random so we could add more values 
    ...
    else if (value == 6) return Tetromino.n(); //[B4] here i add my Tetromino to the pickRandom Method so that it can show up in the game.
